...
const ApplianceSelection = dynamic(
  () => import("@/components/ApplianceSelection"),
  {
    loading: () => (
      <div className="p-8">
        <Skeleton className="h-64 w-full" />
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (!session) {
        ...
        // Load saved data
        const { data: savedData, error: loadError } = await loadTraining(
          session.user.id,
        );
        ...
        if (!currentData) {
          ...
          if (
            currentData.appliance_usage?.history?.length > 0 &&
            !isNewAssessment
          ) {
        ...
        if (currentData) {
          ...
          setData((prev) => ({
            ...
            appliances: Array.isArray(currentData.selected_appliances)
              ? currentData.selected_appliances
              : [],
  ...
  if (!data.mode) {
    return (
      <div className="relative">
        ...
        <div>
          <ModeSelection
            onSelect={(mode) => setData((prev) => ({ ...prev, mode }))}
          />
        </div>
      </div>
    );
    ...
    <div className="min-h-screen relative">
      ...
      {step === 2 && (
        <ApplianceSelection
          ...
          onDetailsUpdate={(d) =>
            setData((prev) => ({ ...prev, details: { ...prev.details, ...d } }))
          }
          onNext={() => {
            if (data.mode === "quick") {
              setStep(4);

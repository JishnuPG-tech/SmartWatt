    ...
    try {
      ...
      for (let i = 0; i < itemsToPredict.length; i++) {
        ...
        const payloadDetails: any = {
          ...
          location_type:
            household.location_type ||
            (household.house_type === "independent" ? "rural" : "urban"),
        };
        ...
        if (name === "ac" || name === "air_conditioner") {
          ...
          payloadDetails.ac_usage_pattern =
            details.ac_usage_pattern || details.ac_pattern || "moderate";
        } else if (name === "fridge" || name === "refrigerator") {
          payloadDetails.fridge_hours = getNum("fridge_hours", 24); // Map hours
          payloadDetails.fridge_capacity_liters = parseFloatVal(
            "fridge_capacity",
            250,
          );
          payloadDetails.fridge_age_years =
            details.fridge_age === "<1" ? 0.5 : parseFloatVal("fridge_age", 5);
          payloadDetails.fridge_star_rating = parseStar("fridge_star");
          payloadDetails.fridge_type =
            details.fridge_type === "frost" ? "frost_free" : "direct_cool";
          payloadDetails.refrigerator_usage_pattern =
            details.fridge_pattern || "always";
        } else if (name === "washing_machine") {
          // WM defines usage by cycles, but we can also map hours if needed
          payloadDetails.wm_cycles_per_week = parseFloatVal(
            "wm_cycles_per_week",
            4,
          );
          ...
          payloadDetails.led_lights_usage_pattern =
            details.led_pattern || "evening";
          ...
          payloadDetails.tube_lights_usage_pattern =
            details.tube_pattern || "evening";
          ...
          payloadDetails.cfl_lights_usage_pattern =
            details.cfl_pattern || "evening";
          ...
          payloadDetails.water_heater_capacity_liters = parseFloatVal(
            "geyser_capacity",
            15,
          );
          payloadDetails.water_heater_type = details.geyser_type || "instant";
          payloadDetails.geyser_usage_pattern =
            details.geyser_pattern || "light";
        } else if (name === "desktop") {
          payloadDetails.desktop_hours = getNum("desktop_hours", 2); // Map hours
          payloadDetails.desktop_usage_pattern =
            details.desktop_pattern || "moderate";
        } else if (name === "laptop") {
          payloadDetails.laptop_hours = getNum("laptop_hours", 4); // Map hours
          payloadDetails.laptop_usage_pattern =
            details.laptop_pattern || "moderate";
        } else if (name === "iron") {
          ...
          payloadDetails.pump_usage_pattern =
            details.pump_pattern || "moderate";
          ...
          payloadDetails.food_processor_hours = getNum(
            "food_processor_hours",
            0.2,
          ); // Map hours
      ...
      for (const name of itemsToPredict) {
        try {
          ...
          if (
            aiRes.insights?.anomaly &&
            aiRes.insights?.anomaly?.status !== "Normal"
          ) {
          ...
          if (ctx.isExactMode) {
            ...
            const getNum = (key: string, def: number) =>
              details[key] ? Number(details[key]) : def;
            const parseFloatVal = (key: string, def: number) =>
              details[key]
                ? parseFloat(details[key].toString().split(" ")[0]) || def
                : def;

            // Pattern to hours for exact mode
            const patternToHoursExact = (
              appId: string,
              pattern: string | undefined,
              def: number,
            ) => {
            ...
            if (name === "ac") {
              hours = getNum("ac_hours", 6);
              count = getNum("ac_units", 1);
            } else if (name === "ceiling_fan") {
              hours = getNum("fan_hours", 12);
              count = parseFloatVal("num_fans", 3);
            } else if (name === "led_light") {
              hours = getNum("led_hours", 6);
              count = getNum("num_led", 5);
            } else if (name === "tube_light") {
              hours = getNum("tube_hours", 5);
              count = 2;
            } else if (name === "cfl_bulb") {
              hours = getNum("cfl_hours", 5);
              count = 2;
              ...
              hours = getNum(
                "tv_hours",
                patternToHoursExact("tv", tvPattern, 4),
              );
            } else if (name === "washing_machine")
              hours = (parseFloatVal("wm_cycles_per_week", 4) * 1.5) / 7;
            else if (name === "water_heater") hours = getNum("geyser_hours", 1);
            else if (name === "water_pump") hours = getNum("pump_hours", 0.5);
            else if (name === "mixer_grinder")
              hours = getNum("mixer_hours", 0.5);
            else if (name === "microwave")
              hours = getNum("microwave_hours", 0.5);
            else if (name === "kettle") hours = getNum("kettle_hours", 0.5);
            else if (name === "induction")
              hours = getNum("induction_hours", 1.5);
      ...
      const hasExactMode = Object.values(requestMap).some(
        (ctx) => ctx.isExactMode,
      );
      ...
      const sortedBreakdown = [...processedBreakdown].sort(
        (a, b) => b.kwh - a.kwh,
      );

...
export default function ResultsReport({
  ...
  const { loading, progress, results, billDetails, error, runAnalysis } =
    useAnalysisEngine(household, appliances, details, trainingId, mode);

  // --- SIMULATION HOOK ---
  const {
    optimization,
    runSimulation: simulateOptimization,
    isSimulating,
  } = useSimulation(household, details, billDetails);
  ...
  return (
    <div className="w-full flex flex-col min-h-screen bg-[#0a0e27] animate-in fade-in duration-500 pb-[100px]">
        ...
        <div className="bg-[#1e293b]/50 px-6 py-3 border-b border-[#334155] flex justify-between items-center backdrop-blur-sm">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-500">
              Session ID: {trainingId?.slice(0, 8)}...
            </span>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-800/50 rounded-2xl border border-slate-700/50 p-6 flex flex-col justify-center transition-all hover:bg-slate-800 hover:border-slate-600">
              ...
              <p className="text-3xl font-bold text-white mb-1">
                ₹{Math.floor(billDetails.total / 2)}
              </p>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-emerald-400 font-bold text-lg leading-tight">
                    {results?.metrics?.confidence || "98.2"}% Confidence
                  </p>
                  <p className="text-emerald-500/60 text-xs uppercase tracking-wider font-semibold">
                    {results?.metrics?.model || "Physics-Verified Model"}
                  </p>
                </div>
              </div>
        ...
        <div className="bg-[#0f172a] border-t border-[#334155] divide-y divide-[#334155]">
          ...
          <div className="px-6 py-3 bg-blue-950/20 border-b border-blue-500/10">
            <p className="text-[10px] text-slate-400 leading-relaxed text-center">
              <strong className="text-blue-400">Note:</strong> Your bill
              includes standby usage, efficiency loss, and behavior patterns.
              SmartWatt distributes this difference intelligently across
              appliances.
            </p>
          </div>
          ...
          <details className="group px-6 py-2">
            <summary className="flex items-center justify-between gap-2 text-[10px] text-slate-400 hover:text-blue-400 cursor-pointer uppercase tracking-widest transition-colors py-2 list-none">
              <div className="flex items-center gap-2">
                ...
                <span className="px-1.5 py-0.5 bg-slate-800 rounded text-[9px] text-slate-500 group-hover:text-blue-400 transition-colors">
                  Assessment
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-800/30 p-3 rounded-lg border border-slate-700/30">
                  ...
                  <p className="text-lg font-bold text-white">
                    ₹{Math.floor(billDetails.total * 0.965)}
                  </p>
                </div>
                {/* Fuel Surcharge */}
                <div className="bg-slate-800/30 p-3 rounded-lg border border-slate-700/30">
                  ...
                  <p className="text-lg font-bold text-white">
                    ₹{Math.floor(billDetails.total * 0.035)}
                  </p>
                </div>
                {/* Total Charge */}
                <div className="bg-blue-900/20 p-3 rounded-lg border border-blue-500/20">
                  <p className="text-blue-400 text-[10px] uppercase tracking-wider mb-1">
                    Final Bill
                  </p>
                  <p className="text-lg font-bold text-white">
                    ₹{Math.floor(billDetails.total)}
          ...
          <details className="group px-6 py-2">
            <summary className="flex items-center justify-between gap-2 text-[10px] text-slate-600 hover:text-blue-400 cursor-pointer uppercase tracking-widest transition-colors py-2 list-none">
              <div className="flex items-center gap-2">
                ...
                <span className="px-1.5 py-0.5 bg-slate-900 rounded text-[9px] text-slate-500 group-hover:text-blue-400 transition-colors">
                  Logic
                </span>
              </div>
              <ChevronRight className="w-3 h-3 transition-transform group-open:rotate-90" />
                ...
                <table className="w-full text-left text-[10px]">
                  <thead className="bg-slate-900 text-slate-400 uppercase tracking-wider">
                    <tr>
                      ...
                      <th className="px-3 py-2 font-medium text-right text-white">
                        Adjusted
                      </th>
                      ...
                      .map((item: any) => {
                        ...
                        return (
                          ...
                          >
                            <td className="px-3 py-2 text-slate-300 font-medium">
                              {isOverhead ? (
                                <div
                                  ...
                                  title="Overhead reduces as input accuracy improves. Includes wiring loss, inverters, and standby power."
                                >
                            ...
                            <td
                              className={`px-3 py-2 text-right font-mono ${diff > 0.1 ? "text-blue-400" : diff < -0.1 ? "text-orange-400" : "text-slate-600"}`}
                            >
                              {diff > 0 ? "+" : ""}
                              {diff.toFixed(1)}
                            </td>
                ...
                data={[
                  {
                    ...
                    error_y: {
                      type: "data",
                      array: results.breakdown.map(
                        (i: any) =>
                          (i.uncertainty / household.kwh) * billDetails.total,
                      ),
                      visible: true,
                      color: "#94a3b8",
                    },
      ...
      <div className="section bg-gradient-to-br from-[#1e293b] to-[#0f172a] border border-[#334155] p-0 rounded-2xl mb-10 overflow-hidden shadow-2xl shadow-blue-900/10">
        ...
        <div className="p-0">
          {" "}
          {/* No padding for table to go edge-to-edge */}
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-[#1a202c] border-b border-slate-700/50">
                <tr>
                  <th className="text-left py-4 px-6 text-slate-400 font-medium uppercase tracking-wider text-xs">
                    Appliance
                  </th>
                  <th className="text-right py-4 px-6 text-slate-400 font-medium uppercase tracking-wider text-xs">
                    Usage (kWh)
                  </th>
                  <th className="text-right py-4 px-6 text-slate-400 font-medium uppercase tracking-wider text-xs">
                    Confidence
              ...
              <tbody className="divide-y divide-slate-700/30">
                {sortedBreakdown.map((item: any, idx: number) => {
                  const getApplianceIcon = (id: string) => {
                    if (id.includes("ac") || id.includes("air_conditioner"))
                      return <AirVent className="w-4 h-4" />;
                    if (id.includes("fridge") || id.includes("refrigerator"))
                    ...
                    if (
                      id.includes("geyser") ||
                      id.includes("heater") ||
                      id.includes("water_heater")
                    )
                      return <ShowerHead className="w-4 h-4" />;
                    if (id.includes("microwave") || id.includes("oven"))
                      return <Microwave className="w-4 h-4" />;
                    if (id.includes("kettle"))
                      return <Coffee className="w-4 h-4" />;
                    if (id.includes("induction") || id.includes("cooker"))
                    ...
                    if (id.includes("tv") || id.includes("television"))
                      return <Tv className="w-4 h-4" />;
                    if (id.includes("desktop") || id.includes("monitor"))
                      return <Monitor className="w-4 h-4" />;
                    if (id.includes("laptop"))
                      return <Laptop className="w-4 h-4" />;
                    if (id.includes("pump") || id.includes("water"))
                      return <Droplet className="w-4 h-4" />;
                    if (id.includes("iron"))
                      return <Shirt className="w-4 h-4" />;
                    if (id.includes("hair") || id.includes("vacuum"))
                      return <Wind className="w-4 h-4" />;
                    if (id === "system_overhead")
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {item.id === "system_overhead" ? (
                            <div
                              ...
                              title="Overhead includes wiring loss, inverters, and standby power."
                            >
                              <span>{item.name}</span>
                      ...
                      <td className="text-right py-4 px-6 text-slate-200 font-mono">
                        {item.kwh.toFixed(2)}
                      </td>
                      ...
                      <td className="text-right py-4 px-6 text-slate-200 font-mono font-bold">
                        ₹{item.cost}
                      </td>
                    </tr>
                  );
                ...
                <tr className="bg-slate-800/50 border-t border-slate-600/50">
                  <td className="py-4 px-6 text-white font-bold uppercase tracking-wider text-xs">
                    TOTAL
                  </td>
                  <td className="text-right py-4 px-6 text-white font-bold font-mono">
                    {household.kwh.toFixed(1)}
                  </td>
                  <td className="text-right py-4 px-6"></td>
                  <td className="text-right py-4 px-6 text-white font-bold font-mono">
                    100%
                  </td>
                  <td className="text-right py-4 px-6 text-white font-bold font-mono text-lg">
                    ₹{Math.floor(billDetails.total)}
                  </td>
                </tr>
              </tbody>
        ...
        <div className="p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {smartInsights.slice(0, 6).map((insight, idx) => (
              <div
                key={idx}
                className="flex gap-3 items-start bg-[#1a202c]/50 p-4 rounded-xl border border-slate-700/50 hover:bg-slate-800/50 hover:border-blue-500/30 transition-all duration-300 group"
              >
                <span className="text-xl mt-0.5 filter drop-shadow-lg group-hover:scale-110 transition-transform">
                  {insight.icon}
                </span>
            ...
            {smartInsights.length > 6 && (
              <div className="col-span-1 md:col-span-2 text-center mt-2">
                <span className="text-xs text-slate-500 italic bg-slate-800/30 px-3 py-1 rounded-full border border-slate-700/30">
                  + {smartInsights.length - 6} more insights available in PDF
                  Report
                </span>
              </div>
            )}
      ...
      <div className="mb-8">
        <BenchmarkCard
          householdKwh={household.kwh}
          numPeople={household.num_people}
        />
      </div>
          ...
          {!optimization ? (
            <div className="text-center py-4">
              <p className="text-slate-400 mb-6 max-w-xl mx-auto text-sm leading-relaxed text-center">
                Leverage our AI-driven simulation engine to forecast potential
                savings. optimize your consumption patterns and visualize the
                impact on your KSEB bill structure.
              </p>
              <button
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="p-4 bg-slate-800/30 rounded-xl border border-slate-700/30 flex flex-col items-center text-center">
                  ...
                  <p className="text-3xl font-bold text-white mb-1">
                    ₹{Math.round(optimization.originalBill)}
                  </p>
                  <p className="text-xs text-slate-500 font-mono">
                    {optimization.originalKwh.toFixed(0)} Units
                  </p>
                </div>
                ...
                <div className="p-4 bg-green-900/10 rounded-xl border border-green-500/30 flex flex-col items-center text-center relative overflow-hidden">
                  ...
                  <p className="text-green-400 text-[10px] uppercase tracking-wider mb-2 relative z-10">
                    Optimized Bill
                  </p>
                  <p className="text-4xl font-bold text-green-400 mb-1 relative z-10 drop-shadow-[0_0_10px_rgba(74,222,128,0.3)]">
                    ₹{Math.round(optimization.newBill)}
                  </p>
                  <p className="text-xs text-green-300/70 font-mono relative z-10">
                    {optimization.newKwh.toFixed(0)} Units
                  </p>
                </div>

                {/* Savings */}
                <div className="p-4 bg-yellow-900/10 rounded-xl border border-yellow-500/30 flex flex-col items-center text-center">
                  <p className="text-yellow-500 text-[10px] uppercase tracking-wider mb-2">
                    Potential Savings
                  </p>
                  <p className="text-3xl font-bold text-yellow-400 mb-1">
                    ₹{Math.round(optimization.savedAmount)}
                  </p>
                  <span className="inline-block px-2 py-1 bg-yellow-500/20 rounded text-[10px] text-yellow-300 border border-yellow-500/20">
                    Save{" "}
                    {(
                      (optimization.savedAmount / optimization.originalBill) *
                      100
                    ).toFixed(0)}
                    %
                  </span>
              ...
              <div className="text-center">
                ...
                <div className="flex flex-wrap gap-3 justify-center">
                  {optimization.breakdown.map((item: string, idx: number) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 bg-slate-800/50 text-slate-300 px-4 py-2 rounded-lg border border-slate-700 hover:border-green-500/50 hover:bg-slate-800 transition-colors text-xs shadow-sm"
                    >
